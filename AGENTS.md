...
For frontend workflows, cache only `.cache/aube` and set `AUBE_STORE_DIR` / `AUBE_CACHE_DIR` into that directory. Never cache `node_modules`, `node_modules/.aube`, or project virtual-store links on Windows runners; those paths are regenerated by `aube install` and can restore broken junctions or symlinks that fail with `EPERM` during Vue/Vite resolution.
